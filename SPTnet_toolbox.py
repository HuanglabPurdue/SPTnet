...
class SPTnet_toolbox(object):
    def __init__(self, *args, **kwargs):
        ...
        image_size =  path_saved_model = use_gpu = number_of_frame = num_queries = None
    ...
    class Transformer_mat2python(torch.utils.data.Dataset):
        def __init__(self, SPTnet_toolbox, dataset_path,num_queries):
            ...
            for i in range(0,len(Hlabel_ref)):
                if np.array(self.dataset[Hlabel_ref[i]][0])!= 0:
                    ...
                    if np.array(self.dataset[position_ref[i]]).T.size == video.shape[0]*2:
                        ...
                        j = j+1
            class_label_pd = np.pad(class_label,[(0,0), (0, self.num_queries- Hlabel.shape[0])], 'constant', constant_values=(0))
            padding_config = ((0, 0), (0, self.num_queries - Hlabel.shape[0]), (0, 0))
            position_pd = np.pad(position, padding_config, 'constant',constant_values=np.nan)
            outfov_mask = np.any((position_pd<-32)|(position_pd>32),axis=2) #when particles move outside the FOV, make the class label as 0
    ...
    def data_loader(self, dataserver_train, split_chunks_for_validation):
        ...
        self.train_dataloader = torch.utils.data.DataLoader(dataserver_train,
                           batch_size= self.batch_size,shuffle=True, num_workers=0, drop_last=True) #transforms.Normalize
    ...
    class BackBone(nn.Module):
        def __init__(self):
            ...
            self.layer2 = self.make_layer(64, 2, stride=(1, 2, 2))
            self.layer3 = self.make_layer(128, 2, stride=(1, 2, 2))
            self.layer4 = self.make_layer(256, 2, stride=(1, 2, 2))
        ...
        def make_layer(self, out_channels, num_blocks, stride):
            ...
            for _ in range(1, num_blocks):
                layers.append(SPTnet_toolbox.ResidualBlock(out_channels, out_channels, stride=(1, 1, 1)))
    ...
    class SPTnet(nn.Module):
        def __init__(self, num_classes, num_queries, num_frames, spatial_t, temporal_t, input_channel):
            ...
            self.conv_temp = nn.Conv1d(1, 30, kernel_size=1, stride=1, padding=0)
            # position encoding
